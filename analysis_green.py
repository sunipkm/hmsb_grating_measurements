...
# %%
data = np.loadtxt('data/meas_3.csv', delimiter=',', dtype=float).T
# %%
...
def costfn(x: Tuple[Numeric, Numeric], angles, lam: Numeric = 633e-6, order: int = 1) -> Numeric:
    ...
    return np.sum(ret**2)
# %%
ret = minimize(partial(costfn, lam=543.5e-6), x0 = (1/98, 90), args=(proc,), bounds=((1/1000, 1), (-180, 180)))
# %%
...
# %%
ret = minimize(partial(costfn, lam=543.5e-6, order=2), x0 = (1/98, 90), args=(proc[::2],), bounds=((1/1000, 1), (-180, 180)))
# %%
print(f"Grating pitch: {ret.x[0]:.4f} mm, Grating density: {1/ret.x[0]:.2f} lines/mm")
plt.plot(proc[:-1], func(ret.x[0], ret.x[1], proc[:-1], proc[1:], order=1), 'o', label='Data')
# %%
